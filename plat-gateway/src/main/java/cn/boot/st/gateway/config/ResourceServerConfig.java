...
import cn.boot.st.gateway.security.AuthorizationManager;
...
import static cn.boot.st.gateway.constant.GatewayErrorCodeConstants.USER_ACCESS_UNAUTHORIZED;
        ...
        return (exchange, denied) -> {
            Mono<Void> mono = Mono.defer(() -> Mono.just(exchange.getResponse()))
                    .flatMap(response -> {
                        ...
                        String body = JSONUtil.toJsonStr(CommonResult.error(USER_ACCESS_UNAUTHORIZED.getCode(), USER_ACCESS_UNAUTHORIZED.getMessage()));
